...
const GeoMap = ({ username, coords }) => {
  ...
  const [socketCoords, setSocketCoords] = useState([]);
  ...
  useEffect(() => {
    ...
    // The client requests existing markers when it connects
    socket.emit("requestCoordinates");

    socket.on("coordinates", (coordinates) => {
      setSocketCoords(coordinates);
      console.log("Updated Coordinates from Socket => ", coordinates);
    });
  ...
  const handleSendMessage = () => {
    if (coords.lat && coords.lng && message.trim() !== "") {
      socket.emit("refreshCoordinates", username, coords, message);
      setMessage("");
      setAnchorEl(null);
    }
  };
  ...
  return (
    <div className={classes.container}>
      ...
      >
        ...
        />
        {socketCoords.map(({ username, coordinates: { lat, lng }, message }, index) => (
