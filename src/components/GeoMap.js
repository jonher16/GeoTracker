...
const GeoMap = ({ username, coords }) => {
  ...
  useEffect(() => {
    socket.on("coordenadas", (coordinates) => {
      ...
      console.log("Nuevos Usuario Desde el Socket => ", coordinates);
    });
    return () => {
      socket.off("coordenadas");
    };
  }, [socketCoords]);
    ...
    <div>
      ...
      >
        ...
        />
        {
           ...
           socketCoords && socketCoords.map( ({username, coordinates: {lat, lng}}, index) => (
              <Marker
                key={index}
                icon={myIcon}
